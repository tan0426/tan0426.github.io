...
```abap
SELECTION-SCREEN BEGIN OF LINE.
 SELECTION-SCREEN COMMENT 1(10) TEXT-A02 FOR FIELD P_RADI1.
 PARAMETERS : P_RADI1 RADIOBUTTON GROUP RADI DEFAULT 'X' USER-COMMAND RADI.
SELECTION-SCREEN END OF LINE.

SELECTION-SCREEN BEGIN OF LINE.
 SELECTION-SCREEN COMMENT 1(10) TEXT-A03 FOR FIELD P_RADI2.
 PARAMETERS : P_RADI2 RADIOBUTTON GROUP RADI.
SELECTION-SCREEN END OF LINE.
```

AT SELECTION-SCREEN OUTPUT에서 RADIO버튼동작을 제어 할 수 있다.

```abap
AT SELECTION-SCREEN OUTPUT.
  PERFORM RADI_CONTROL.
```
...
## FUNCTION KEY
INITIAL에 FUNCTION KEY를 추가

```abap
SELECTION-SCREEN FUNCTION KEY 1.
```

### INITIAL에 FUNCTION KEY의 아이콘과 이름을 추가

```abap
DATA : GV_ICON TYPE ICON-ID.
```

```abap
FORM FUNCTION_KEY .
  PERFORM GET_ICON USING 'ICON_XLS' CHANGING GV_ICON.

  CONCATENATE GV_ICON 'DOWNLOAD EXCEL' INTO SSCRFIELDS-FUNCTXT_01.
ENDFORM.                    " FUNCTION_KEY
```

```abap
FORM GET_ICON USING PV_NAME CHANGING PV_ICON.
  SELECT SINGLE ID
    INTO PV_ICON
    FROM ICON
   WHERE NAME = PV_NAME.
...
### AT SELECTION-SCREEN 에 USER COMMAND 적용
